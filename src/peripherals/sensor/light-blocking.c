/** 
 * @file        light-blocking.c
 * @brief       Contains low level functions for the light-blocking
 *              sensor. 
 *
 * @author Yogesh Hegde
 */
...
/**
 * @brief      Initializes the light-blocking sensor
 *
 * @param[in]  pin   The physical pin number that the sensor is connected.
 */
static void light_blocking_begin(uint8_t pin)
{
 ...
 */
uint8_t read_light_blocking(uint8_t pin) 
{       
        /* Initialize the sensor */        
        light_blocking_begin(pin);
        /* Read the sensor */
